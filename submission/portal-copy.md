...
We taught NVIDIA SONIC a five-second **Shadow Partner Dip + Gancho**: the G1 establishes
an absent-partner ballroom frame, steps back and pivots, transfers into an unsupported
off-axis one-foot hold, sweeps the free leg into a 29–31 cm hooked pose, and recovers
without a partner, hand support, or floor contact. The authored family reaches a
14.7 cm pelvis drop and 28.4-degree waist roll; all 54 generated references pass the
manifest-bound kinematic checks with zero warnings or self-contacts.

This is not a generic “robot dances” entry. SONIC already knows broad dance motion. Our
test is the exact unsupported sequence, including the aerial hook and recovery. After
selecting the checkpoint on separate validation motions, we opened an untouched
final-test family. Across 24 matched test trials per policy (8 motions × 3 simulator
seeds), stock SONIC completed **[x/24]** versus **[y/24]** for our selected checkpoint;
...
Our 54-sequence dataset is team-authored and reproducible: 12 training dips, 12
training ganchos, 10 conservative fundamentals rehearsals, eight independently
parameterized validation motions, four disclosed preflight-only legacy motions, and
eight fresh final-test motions. The preflight motions are excluded from training,
selection, and final reporting. MuJoCo inverse kinematics pins the stance feet and
landings against NVIDIA's official G1 model. A public manifest records every
specification, phase, upstream commit, IK residual, and SHA-256. No BONES-SEED motion
or third-party video is included.
...
- Immutable v2 dataset fallback: `https://github.com/cristpierce/shadow-dance/releases/tag/shadow-dance-v2.0.0`
...
- [ ] Headline numbers come from all 24 untouched final-test trials bound to selection.
